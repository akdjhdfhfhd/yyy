...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"**گۆرانییەکە لەو شوێنەی کە وەستابوو دەستی پێکردووەتەوە ⚡.**\n\n /resume ئەگەر دەتەوێت بیوەستێنیت بنێرە"
            )
        except Exception as e:
            await m.reply(f"**ERROR** \n`{e}`")
    else:
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"**گۆرانییەکە بە سەرکەوتوویی ڕاگیراوە ⚡️**\n\nئەگەر دەتەوێت دووبارە دەستپێبکەیتەوە بنووسە  {HNDLR} دووبارە دەستپێکردنەوە**"
            )
        except Exception as e:
            await m.reply(f"**ERROR** \n`{e}`")
    else:
